export default async function handler(req, res) {
  ...
  try {
    ...
    const allowedDomains = new Set(['linkvertise.com','lootlinks.com','admaven.com','work.ink','shortearn.eu','beta.shortearn.eu','cuty.io','ouo.io','lockr.so','rekonise.com','mboost.me','link-unlocker.com','mega.nz','mega.co.nz']);
    if (!allowedDomains.has(hostname)) return res.status(403).json({ result: 'Unsupported shortener. Only supported: Linkvertise, LootLabs, AdMaven, Work.ink, Beta.shortearn.eu, Cuty.io, ouo.io, Lockr, Rekonise, MBoost.me, link-unlocker.com, mega.nz', status: 'error', time: '0.00' });
    ...
    let initialData;
    try { initialData = await initialRes.json(); } catch { return res.status(502).json({ result: 'Invalid response from TRW', status: 'error', time: ((Date.now()-startTime)/1000).toFixed(2) }); }
    if (!initialData.success || !initialData.task_id) return res.status(200).json({ result: initialData.result || 'Failed to start task', status: 'error', time: ((Date.now()-startTime)/1000).toFixed(2) });
